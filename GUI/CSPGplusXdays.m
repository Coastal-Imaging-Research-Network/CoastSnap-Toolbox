function CSPGplusXdays(handles)
...
Inew = interp1(data.navigation.epochs,1:length(data.navigation.epochs),data.epoch+timestep_seconds,'nearest','extrap');
newpath = data.navigation.paths(Inew).name; %Step one forward
newfile = data.navigation.files(Inew).name; %Step one forward

if Inew==I
    warning('The image found is the same as the present image. Try increasing the timestep to search further into the future')
else
